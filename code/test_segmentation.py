...
def compute_iou(pred, target, num_classes=10, ignore_index=255):
    ...
    Now skips background class (class 0) which is artificially created.
    """
    ...
    for class_id in range(num_classes):
        ...
        # SKIP BACKGROUND (class 0) - it's unmapped pixels, not a real class
        if class_id == 0:
            iou_per_class.append(float('nan'))
            continue
        ...
        # Skip if class doesn't exist in ground truth
        ...
        # Only count valid IoUs for mean
        if not np.isnan(iou):
    ...
    return mean_iou, np.array(iou_per_class)
